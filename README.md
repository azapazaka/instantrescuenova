...
## 1. Проблема (GovTech-контекст)

Болезни системы кровообращения (БСК) — **причина смертности №1 в Казахстане**.
За 2024 год от БСК скончались **29 800 человек**, из них **9 400** — от
сосудистых поражений мозга (инсультов) ([stat.gov.kz](https://stat.gov.kz),
данные Бюро национальной статистики). Каждый случай инсульта обходится
государству в **230 000–550 000 ₸** (стационар + реабилитация через ОСМС),
а у выживших часто наступает инвалидность, требующая многолетних
социальных выплат. Большинство этих случаев можно было бы предотвратить
ранним выявлением нарушений ритма — но у государственной системы
здравоохранения физически не хватает ресурсов для непрерывного мониторинга
каждого пациента из группы риска.

Три конкретные точки отказа в текущей системе, которые мы закрываем:

1. **Слепая зона между визитами в поликлинику.** Мерцательная аритмия часто
   проходит без симптомов, и человек попадает в больницу уже с инсультом.
   Государство тратит сотни тысяч тенге на экстренное лечение и квоты по
   инвалидности — вместо дешёвого превентивного мониторинга.
2. **Перегрузка врачей ВОП.** Пожилой человек получает PDF с анализами
   (например, через eGov Health или лабораторию) и не понимает их. Из-за
   этого он либо упускает болезнь, либо лишний раз записывается в
   поликлинику просто «чтобы спросить», создавая очереди.
3. **Падение = потерянное время для скорой (103).** Падение может быть
   следствием инсульта. Человек лежит один, а экстренные службы не знают
   об этом часами, теряя «терапевтическое окно» для спасения.

**Почему эту проблему нельзя решить обычной автоматизацией?** Обычная
автоматизация (напоминания, формы, таймеры) не умеет отличать нормальный
ритм от аномального — для этого нужен ML, обученный на реальных
кардиологических данных. Объяснение медицинских анализов простым языком
требует NLP/LLM с привязкой к клиническим источникам (RAG). А детекция
падения по данным акселерометра — задача классификации сигналов, которую
правилами не закрыть.

## 2. Пользователи и сценарий внедрения

Наше решение спроектировано как **GovTech-инструмент превентивного
здравоохранения**, который снижает нагрузку на государственную систему
здравоохранения, работая напрямую с гражданами из группы риска.

**Конечные пользователи (Граждане):**
1. Пожилой человек (65+), который живёт один или частично самостоятельно.
2. Его близкий родственник, получающий уведомления.

**Пользователи со стороны государства (GovTech-аудитория — косвенные выгодоприобретатели):**
1. **Система здравоохранения (поликлиники)**: снижение нагрузки на врачей (ВОП) за счет превентивного выявления рисков.
2. **Служба скорой помощи (103)**: повышение скорости реагирования и снижение ложных вызовов за счет точных данных, передаваемых от близких пациента.

Сценарий:

1. Пожилой человек открывает приложение и видит свой пульс сейчас.
2. Система оценивает ритм окнами по 5 минут и объясняет, **какие именно показатели** отклонились от нормы.
3. Раз в день пользователь заполняет короткую форму — получает план на день со ссылками на рекомендации ВОЗ/МЗ РК.
4. Загружает PDF с анализами (с eGov) — получает объяснение простым языком, что снижает потребность в лишнем визите к врачу.
5. Если датчик фиксирует падение — близкий получает сообщение в Telegram со временем события, чтобы экстренно вызвать скорую (103).
...
| Снижение нагрузки на ВОП | Снижение нецелевых обращений за счет понятного объяснения анализов |

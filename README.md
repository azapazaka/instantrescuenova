...
Сердечно-сосудистые заболевания (ССЗ) — главная причина смертности в Казахстане, создающая колоссальную нагрузку на **государственную систему здравоохранения** и экстренные службы (скорую помощь, реанимации). Большинство преждевременных смертей и инсультов можно предотвратить ранним выявлением, однако у государства физически не хватает ресурсов для непрерывного мониторинга каждого пациента из группы риска.

Три конкретные точки отказа в текущей системе, которые мы закрываем:

1. **Слепая зона между визитами в поликлинику.** Ухудшение замечают поздно. Мерцательная аритмия часто проходит без симптомов, и человек попадает в больницу уже с инсультом, что требует огромных затрат государства на лечение и квоты по инвалидности.
2. **Перегрузка врачей ВОП.** Пожилой человек получает PDF с анализами (например, с eGov Health или лаборатории) и не понимает их. Из-за этого он либо упускает болезнь, либо лишний раз записывается в поликлинику просто «чтобы спросить», создавая очереди.
3. **Падение = потерянное время для скорой.** Падение может быть следствием инсульта. Человек лежит один, а экстренные службы (103) не знают об этом часами, теряя «золотое окно» (терапевтическое окно) для спасения.

## 2. Пользователи и сценарий внедрения

Наше решение спроектировано как **GovTech-инструмент социальной поддержки** (уровень B2G2C), который может быть интегрирован в работу социальных служб акиматов или городских поликлиник.
